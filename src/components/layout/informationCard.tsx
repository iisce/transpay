...
export function InformationCard({ header, info, link }: INFORMATIONCARDI) {
  return (
    <div className="bg-primary-50 lg:w-[926px] rounded-[20px] p-6 flex flex-col">
      <h3 className="text-h5Bold">{header}</h3>
      <div className="flex justify-between">
        <div className="">
          {info.map((i, k) => (
            <ProfileInformation title={i.title} entry={i.entry} key={k} />
          ))}
        </div>
        <div className="">
          <Link className="" href={link}>
            <Button className="z-10 w-[100px] mt-auto">
              {editIcon} <p className="ml-2 text-title1Bold">EDIT</p>{" "}
            </Button>
          </Link>
        </div>
      </div>
